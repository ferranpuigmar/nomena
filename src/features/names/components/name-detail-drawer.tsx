...
export function NameDetailDrawer({ name, isOpen, onClose, onPrev, onNext }: NameDetailDrawerProps) {
  return (
              ...
              <div className="flex h-full flex-col overflow-y-auto bg-white shadow-xl">
                ...
                {name && (
                  <div className="flex-1 px-6 py-6 space-y-6">
                    ...
                    {name.meaning && (
                      <div>
                        <h3 className="text-sm font-medium text-gray-500 mb-1">Significado</h3>
                    ...
                    {name.usageScore != null && (
                      <div>
                        <h3 className="text-sm font-medium text-gray-500 mb-2">Personas registradas</h3>
                            ...
                            {Array.from({ length: 5 }, (_, i) => (
                              <span
                                ...
                                className={`h-3 w-3 rounded-full ${
                                  i < scoreToDots(name.usageScore!) ? 'bg-gray-700' : 'bg-gray-200'
                                }`}
                              />
                            ))}
                          </div>
                    ...
                    {name.spainUsageRank != null && (
                      <div>
                        <h3 className="text-sm font-medium text-gray-500 mb-1">Ranking en España</h3>
                        <p className="text-sm text-gray-900">#{name.spainUsageRank}</p>
                      </div>
                    )}

                    {/* Length */}
                    <div>
                      <h3 className="text-sm font-medium text-gray-500 mb-1">Longitud</h3>
                      <p className="text-sm text-gray-900">
                        {name.length} letras · {name.lengthCategory === 'short' ? 'Corto' : 'Largo'}
                      </p>
                    </div>
                  </div>
                )}
